  ...
  return (
    <>
      <tr className="border-b border-slate-100">
        <td className="px-3 py-2 font-mono text-xs text-slate-500">{row.seq}</td>
        ...
        <td className="px-3 py-2">
          ...
          {row.actor_employee_code ? (
            <div className="text-xs text-slate-500">{row.actor_employee_code}</div>
          ) : null}
          {row.actor_ip ? (
            <div className="font-mono text-xs text-slate-400">{row.actor_ip}</div>
          ) : null}
        </td>
        <td className="px-3 py-2">
        ...
        <td className="px-3 py-2 text-xs text-slate-600">
          {row.entity_type}
          <div className="font-mono text-slate-400">{row.entity_id.slice(0, 8)}…</div>
        </td>
        <td className="px-3 py-2">
          {hasDiff ? (
            <button
    ...
    <div className="flex flex-wrap items-end gap-3 rounded-md border border-slate-200 bg-white p-3">
      ...
      <label className="block">
        ...
        <input
          ...
          className="mt-1 block rounded-md border border-slate-300 px-3 py-1.5 font-mono text-sm"
        />
      </label>
...
function Anchors() {
  ...
  return (
    <section className="rounded-md border border-slate-200 bg-white p-4">
      <h2 className="text-sm font-semibold text-slate-900">Nightly notarisations</h2>
      ...
      <ul className="mt-3 space-y-1 text-xs">
        {data.slice(0, 10).map((anchor) => (
          <li key={anchor.anchored_at} className="flex flex-wrap gap-2">
            ...
            <span className={anchor.chain_intact ? "text-green-800" : "font-semibold text-red-800"}>
              {anchor.chain_intact ? "intact" : `BROKEN at ${anchor.first_break_seq}`}
            </span>
            <span className="text-slate-500">seq {anchor.head_seq}</span>
